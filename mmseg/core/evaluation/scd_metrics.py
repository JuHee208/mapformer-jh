...
def scd_eval_metrics(
    ...
    ret_metrics['BC'] = (total_bc_intersect / (total_bc_union + eps)).item()
    ret_metrics['SC'] = (total_sc_intersect / (total_sc_union + eps)).mean()
    ret_metrics['mIoU'] = (total_sem_intersect / (total_sem_union + eps)).mean()
    # Precision = TP / (TP + FP)  => TP / Pred_Pos
    # Recall    = TP / (TP + FN)  => TP / GT_Pos
    ret_metrics['BC_recall'] = (total_bc_intersect / (total_bc_gt + eps)).item()
    ret_metrics['BC_precision'] = (total_bc_intersect / (total_bc_pred + eps)).item()
